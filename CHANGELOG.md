...
- 同步重写 [README.md](README.md) 为当前版本的最终形态说明：围绕「记录 / 对话 / 洞察」三主 Tab、CBT 报告结构、`ConversationSeed.autoSend` 行为、深浅色主题、移动端「沉淀 Echo」、最新 API 与数据模型。
- README 目录、项目结构、功能说明、排错与部署建议已与当前代码状态对齐，减少“文档描述与实现不一致”的问题。
